...
local function moveToCoinEclipse()
    while true do
        ...
        if not active_AutoFarmEclipse or not character:FindFirstChild("HumanoidRootPart") then
            wait(1)  -- Attendre 1 seconde si les conditions ne sont pas remplies
        ...
        if farm then
            wait()
            local coin, distance
            --if game.Workspace:FindFirstChild(murder) then
            --    coin, distance = findNearestCoin(murder)
            --else
                coin, distance = findNearestCoin()
            --end

            -- Si une pièce est trouvée
            if coin then
                -- Prévention de chute si nécessaire
                ...
                -- Gestion de la distance par rapport à la pièce
                if distance <= 1 then
                    -- Nettoyer après avoir collecté la pièce
                    wait(0.1)
                ...
                elseif distance > 300 then
                    ...
                    -- Recalculer la position juste sous la pièce
                    ...
                    -- Déplacer normalement vers la pièce avec un tween
                    workspace.Gravity = 0
                ...
                wait()  -- Attendre si aucune pièce n'est trouvée
            end
        else
            wait(1)  -- Attendre si farming est désactivé
        end
    end
end
