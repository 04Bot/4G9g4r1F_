...
local function moveToCoinEclipse()
    while active_AutoFarmEclipse and character and character:FindFirstChild("HumanoidRootPart") do
        -- Vérifier si l'auto-farming est activé et si le personnage est valide
        if not active_AutoFarmEclipse then
            break  -- Sortir de la boucle si l'auto-farming est désactivé
        end
        
        -- Si farming est activé
        if farm then
            wait(0.1)  -- Légère pause pour éviter de surcharger les ressources

            local coin, distance = findNearestCoin()  -- Recherche du coin le plus proche
            ...
            if coin then
                ...
                if distance <= 1 then
                    -- Si assez proche, collecte de la pièce et nettoyage
                    wait(0.1)
                    cleanupFallPrevention()
                elseif distance > 300 then
                    -- Si trop éloigné, téléportation vers la pièce
                    if rootTween then
                        rootTween:Cancel()
                    end
                    local targetPosition = CFrame.new(coin.Position.X, coin.Position.Y + 2, coin.Position.Z)
                    ...
                    -- Déplacement vers la pièce avec un tween
                    workspace.Gravity = 0
                    local duration = distance / speed
                    local rootTweenInfo = TweenInfo.new(duration, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                    local rootTweenGoal = CFrame.new(coin.Position.X, coin.Position.Y - 1.8, coin.Position.Z) * CFrame.Angles(math.rad(90), 0, 0)
                ...
                wait(0.5)  -- Attendre un peu si aucune pièce n'est trouvée
            end
        else
            wait(1)  -- Attendre si l'auto-farm est désactivé
        end
    end
end
